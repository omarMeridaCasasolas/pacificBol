...
<body class="hero-anime">	
    ...
    <div class="container mt-5 bg-info rounded p-3">
		...
		<div class="text-center">
            <img src="src/img_importacion.jpg" alt="" srcset="" width="70%">
        </div>
    </div>
</body>
		...
		$.ajax({
			...
			success: function (response) {
				...
				if(listaTramite.length == 0){
                    $("#contenorImportacion").append("<div class='text-center'><img src='src/sin_publicaciones.gif' alt='imagen sin contenido'></img></div>");
                }
				listaTramite.forEach(element => {
					$("#contenorImportacion").append("<a href='"+element.enlace+"' class='text-white' target='_blank'>"+element.nombre+" <i class='fas fa-download'></i> </a><p class='text-justify'>"+obtenerRespuesta(element.descripcion)+"<p><hr>");
				});
			}
		});    
	}
